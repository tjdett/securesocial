 ...
 */
case class Authenticator(id: String, userId: UserId, expirationDate: DateTime)
{
   ...
   */
  def isValid: Boolean = !expirationDate.isBeforeNow
}
...
object Authenticator {
  ...
  def create(user: Identity): Either[Error, Authenticator] = {
    ...
    val authenticator = Authenticator(id, user.id, expirationDate)
